...
def serve_layout():
    return html.Div([
        html.H3("App Data Visualization", style={'textAlign': 'center'}),
...
def update_graph(selected_categories, selected_content_rating, free_paid, app_age_range, size):
    filtered_df = df[df['Category'].isin(selected_categories) &
                     ...
                     (df['Free'].astype(str) == free_paid) &
                     (df['App Age'] >= app_age_range[0]) &
                     (df['App Age'] <= app_age_range[1]) &
                     (df['Size in MB'] <= size)]

    # Generating the graph
    fig = px.scatter_3d(filtered_df, x='Rating', y='Size in MB', z='Installs', color='Category', size='Installs',)
